...
function ApproveAccounts() {
    ...
    const [gotUnapprovedAccounts, setGotUnapprovedAccounts] = useState(false);
    const [unapprovedAccounts, setUnapprovedAccounts] = useState([]);

    if (gotUnapprovedAccounts === false) {
        setGotUnapprovedAccounts(true);
        axios.get("/admin/users/unapproved", {
            headers: {
                "Authorization": "Bearer " + accessToken
            }
        }).then((response) => {
            setUnapprovedAccounts(response.data.users);
        }).catch((error) => {
    ...
    return (
        <div id="manage-accounts" className="container fluid my-4">
